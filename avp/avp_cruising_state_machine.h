...
private: 
    ...
    StateMachineBase<AvpCruisingStateType>("AvpCruising", std::make_shared<AvpCruisingParam>(), std::make_shared<AvpCruisingInput>(), std::make_shared<AvpCruisingSwitch>(), std::make_shared<AvpCruisingOutput>())
    {

    }
    virtual void PrintStateSwitchInfo() override {};
    virtual AvpCruisingStateType CalcNextState() const noexcept override{ return AvpCruisingStateType::IDLE_0; };
};
};
